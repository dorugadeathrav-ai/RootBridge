...
export default function AddProductPage() {
  ...
  const handleImageFiles = (files: FileList | null) => {
    ...
    Array.from(files).forEach(file => {
      ...
      reader.onload = () => {
        setImages(previous => [...previous, String(reader.result)])
      }
  ...
  const handleSubmit = (event: React.FormEvent) => {
    ...
    if (!form.name.trim() || !form.category.trim() || !form.price || !form.quantity) {
      setMessage('Please fill in the required product information.')
      return
    }

    const product: SellerProduct = {
      id: typeof crypto.randomUUID === 'function' ? crypto.randomUUID() : `product-${Date.now()}`,
    ...
    saveProduct(product)
    navigate('/seller-dashboard/products', { replace: true, state: { successMessage: 'Product saved successfully.' } })
  }
            ...
            <form onSubmit={handleSubmit} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
              <div className="mb-6 flex items-center justify-between gap-3">
                <div>
                  ...
                  <h2 className="mt-2 text-2xl font-bold text-slate-900">Create a new listing</h2>
                </div>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <div>
                  ...
                  <input value={form.name} onChange={event => updateField('name', event.target.value)} className="w-full rounded-xl border border-slate-200 px-3 py-2.5 outline-none transition focus:border-emerald-500" placeholder="e.g. Fresh Organic Turmeric" />
                </div>

                <div>
                  <label className="mb-2 block text-sm font-medium text-slate-700">Category *</label>
                  <input value={form.category} onChange={event => updateField('category', event.target.value)} className="w-full rounded-xl border border-slate-200 px-3 py-2.5 outline-none transition focus:border-emerald-500" placeholder="e.g. Spices" />
                </div>

                <div className="md:col-span-2">
                  <label className="mb-2 block text-sm font-medium text-slate-700">Description</label>
                  <textarea value={form.description} onChange={event => updateField('description', event.target.value)} className="min-h-[120px] w-full rounded-xl border border-slate-200 px-3 py-2.5 outline-none transition focus:border-emerald-500" placeholder="Share product details, quality, and origin." />
                </div>

                <div>
                  <label className="mb-2 block text-sm font-medium text-slate-700">Price *</label>
                  <input type="number" min="0" step="0.01" value={form.price} onChange={event => updateField('price', event.target.value)} className="w-full rounded-xl border border-slate-200 px-3 py-2.5 outline-none transition focus:border-emerald-500" placeholder="0" />
                </div>
                ...
                <div>
                  ...
                  <input type="number" min="0" value={form.quantity} onChange={event => updateField('quantity', event.target.value)} className="w-full rounded-xl border border-slate-200 px-3 py-2.5 outline-none transition focus:border-emerald-500" placeholder="0" />
                </div>
              ...
              <div className="mt-8 flex justify-end gap-3">
                ...
                <button type="submit" className="rounded-lg bg-emerald-600 px-5 py-2.5 text-sm font-semibold text-white hover:bg-emerald-700">Save Product</button>
              </div>
            </form>
          </main>
        </div>
